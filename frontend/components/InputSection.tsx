...
export function InputSection({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalized) {
      setValidationError("Enter a product code before continuing.");
      return;
    }
  ...
  return (
    <section className="mx-auto w-full max-w-3xl rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-soft backdrop-blur">
      ...
      <p className="mt-2 text-sm text-slate-600">
        Submit a product code to start a new run, or load existing workflow history from the database.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-3">
        <input
          ...
          placeholder="Enter product code, e.g. 012345678901 or BOL-12345"
